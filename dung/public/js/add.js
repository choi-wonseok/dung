            ...
            naver.maps.Service.reverseGeocode(
                ...
                function (status, response) {
                    if (status === naver.maps.Service.Status.ERROR) {
                        if (!latlng) {
                            return alert(
                                "ReverseGeocode Error, Please check latlng"
                            );
                        }
                        if (latlng.toString) {
                            return alert(
                                "ReverseGeocode Error, latlng:" +
                                    latlng.toString()
                            );
                        }
                    ...
                    var address2 =
                        dong + " " + (li ? li + " " : "") + jibun + "번지";
                    document
                        .getElementsByName("inputtoiletName")
                        .item(0).value = building;
                    document.getElementsByName("inputaddress").item(0).value =
                        address1;
                    document
                        .getElementsByName("inputaddressDetail")
                        .item(0).value = address2;
                }
            );
        }

        var map = new naver.maps.Map("add-map", {
