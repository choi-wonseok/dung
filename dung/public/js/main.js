...
var coords;
var locationBtnHtml = '<div class="btn_mylct" id="btn_mylct"><a href="#"></a></div>';
			...
			function (position) {
				...
				naver.maps.Event.once(map, "init", function () {
					...
					var myMarker = new naver.maps.Marker({
						...
						map: map,
						icon: {
							content: [`<div class="my-img-content"></div>`].join(""),
				...
				$(".toilet").each(function (index) {
					...
					var a = new naver.maps.LatLng(row.attr("lat"), row.attr("lng"));
					...
					var marker = new naver.maps.Marker({
						position: new naver.maps.LatLng(row.attr("lat"), row.attr("lng")),
						map: map,
						icon: {
							content: [
								`<div class="img-content" id="image_${index}" onmouseover="javascript:overCrime(\'toilet_${index}\');" onmouseout="javascript:outCrime(\'toilet_${index}\');"></div>` +
									...
									`<div class="content-name"> ${row.attr("toiletName")} </div>` +
									`<div class="content-detail"> ${row.attr("toiletDetail")} <br> ${(
				...
				if (!paramsStr) {
					location.href = location.href + `?lat=${coords.latitude}&lng=${coords.longitude}`;
				}
			},
			function (error) {
				console.error(error);
